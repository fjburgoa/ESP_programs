...
void mpu6050_set_accel_scale(uint8_t scale) 
{
    ...
    mpu6050_register_write(0x1C, value); // Escribir en ACCEL_CONFIG
}
...
void mpu6050_set_gyro_scale(uint16_t scale) {
    ...
    mpu6050_register_write(0x1B, value); // Escribir en GYRO_CONFIG
}
...
void mpu6050_set_dlpf(uint8_t bandwidth) 
{
 ...
 //   7	        Reservado	        -	            -
    ...
    mpu6050_register_write(0x1A, bandwidth); // Escribir en el registro CONFIG
}
...
void mpu6050_set_offsets(int16_t *accel_offsets, int16_t *gyro_offsets) {
    ...
    mpu6050_register_write(0x06, (accel_offsets[0] >> 8) & 0xFF); // XA_OFFS_H
    mpu6050_register_write(0x07, accel_offsets[0] & 0xFF);        // XA_OFFS_L_TC
    mpu6050_register_write(0x08, (accel_offsets[1] >> 8) & 0xFF); // YA_OFFS_H
    mpu6050_register_write(0x09, accel_offsets[1] & 0xFF);        // YA_OFFS_L_TC
    mpu6050_register_write(0x0A, (accel_offsets[2] >> 8) & 0xFF); // ZA_OFFS_H
    mpu6050_register_write(0x0B, accel_offsets[2] & 0xFF);        // ZA_OFFS_L_TC
    
    // Escribir offsets del giroscopio
    mpu6050_register_write(0x13, (gyro_offsets[0] >> 8) & 0xFF);  // XG_OFFS_USRH
    mpu6050_register_write(0x14, gyro_offsets[0] & 0xFF);         // XG_OFFS_USRL
    mpu6050_register_write(0x15, (gyro_offsets[1] >> 8) & 0xFF);  // YG_OFFS_USRH
    mpu6050_register_write(0x16, gyro_offsets[1] & 0xFF);         // YG_OFFS_USRL
    mpu6050_register_write(0x17, (gyro_offsets[2] >> 8) & 0xFF);  // ZG_OFFS_USRH
    mpu6050_register_write(0x18, gyro_offsets[2] & 0xFF);         // ZG_OFFS_USRL
}
...
void mpu6050_set_sample_rate(uint16_t rate_hz) 
{
    ...
    mpu6050_register_write(0x19, smplrt_div); // Escribir en SMPLRT_DIV
}