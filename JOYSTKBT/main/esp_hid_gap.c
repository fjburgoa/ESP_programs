...
esp_err_t esp_hid_ble_gap_adv_init(uint16_t appearance, const char *device_name)
{
    ...
    // No entrada ni salida de PIN
    ble_hs_cfg.sm_io_cap       = BLE_SM_IO_CAP_NO_IO;

    // No exigir MITM (evita PIN/passkey)
    ble_hs_cfg.sm_mitm         = 0;

    // Aún puedes mantener bonding si quieres recordar la conexión
    ble_hs_cfg.sm_bonding      = 1;   // 0 si no quieres guardar claves
    ble_hs_cfg.sm_sc           = 0;   // Secure Connections desactivado
